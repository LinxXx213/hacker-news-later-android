        ...
        if (nodeType == Node.ELEMENT_NODE) {
            ...
            if (parentName == "channel") {
                when (nodeName) {
                    ...
                    "itunes:image" -> {
                        rssFeedResponse.image = node.attributes.getNamedItem("href").textContent
                    }
                    "item" -> rssFeedResponse.episodes?.add(RssFeedResponse.EpisodeResponse())
                    "pubDate" -> rssFeedResponse.lastUpdated = HNLDateTime.newInstance().xmlDateToDate(node.textContent)
                }
            }
        }
